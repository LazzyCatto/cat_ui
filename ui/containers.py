...
class Box(ContainerElement):
    ...
    def set_style(self, vertial_bar="|", horizonal_bar="-", corners=["*", "*", "*", "*"], fill=" "):
        self.vertial_bar = vertial_bar
        self.horizonal_bar = horizonal_bar
        self.corners = corners
        self.fill = fill

    def draw(self):
        text = f"{self.vertial_bar}{self.fill*(self.get_width()-2)}{self.vertial_bar}{move_left(self.get_width())}\033[1B"*(self.get_height()-2)
...
class BoxContainer(Box):
    def __init__(self, min_width=3, min_height=3, left_padding=1, right_pading=1, top_pading=1, bottom_pading=1, alignment: Alignment=Alignment.TOP_LEFT, **style):
        super().__init__(min_width, min_height, alignment, **style)
        self.left_padding = left_padding
        self.right_pading = right_pading
        self.top_pading = top_pading
        self.bottom_pading = bottom_pading
    ...
    def get_width(self):
        return max(self.width, self.left_padding + self.right_pading + max([child.get_width() for child in self.children]))
    
    def get_height(self):
        return max(self.height, self.top_pading + self.bottom_pading + max([child.get_height() for child in self.children]))
    
    def draw(self):
        children_draw = "".join([child.draw_on_screen(self.get_width() - self.left_padding - self.right_pading, self.get_height() - self.top_pading - self.bottom_pading) for child in self.children])
        return f"{super().draw()}{move_right(self.left_padding)}{move_down(self.top_pading)}{children_draw}{move_up(self.left_padding)}{move_left(self.left_padding)}"
    ...
    def process_key(self, raw_key):
        ...
        if key_type == KeyType.ARROW:
            if key == Key.UP_ARROW or Key.LEFT_ARROW:
                if self.selection_index > 0:
                    ...
                    return ""
                return raw_key
            if self.selection_index < len(self.children) - 1:
                self.children[self.selection_index].deselect()
                self.selection_index += 1
                self.children[self.selection_index].select()
                return ""
...
class VerticalList(ContainerElement):
    ...
    def get_width(self):
        return max(self.width, max([child.get_width() for child in self.children]))
    
    def get_height(self):
        return max(self.height, sum([child.get_height() + self.space for child in self.children]) - self.space)
...
class HorizontalList(ContainerElement):
    ...
    def get_width(self):
        return max(self.width, sum([child.get_width() + self.space for child in self.children]) - self.space)
    
    def get_height(self):
        return max(self.height, max([child.get_height() for child in self.children]))
...
class VerticalContainer(BoxContainer):
    def __init__(self, min_width=3, min_height=3, left_padding=1, right_pading=1, top_pading=1, bottom_pading=1, space=1, alignment = Alignment.TOP_LEFT, **style):
        super().__init__(min_width, min_height, left_padding, right_pading, top_pading, bottom_pading, alignment, **style)
        self.children = [VerticalList(min_width - left_padding - right_pading, min_height - top_pading - bottom_pading, space)]
...
class HorizontalContainer(BoxContainer):
    def __init__(self, min_width=3, min_height=3, left_padding=1, right_pading=1, top_pading=1, bottom_pading=1, space=1, alignment = Alignment.TOP_LEFT, **style):
        super().__init__(min_width, min_height, left_padding, right_pading, top_pading, bottom_pading, alignment, **style)
        self.child = HorizontalList(min_width - left_padding - right_pading, min_height - top_pading - bottom_pading, space)
